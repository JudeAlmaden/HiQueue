...
import { updateQueueAction } from "@/server/actions/queue.action"
...
export function EditQueueForm({ queue, organizationId, orgSlug, onClose }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    formData.set("id", queue.id)
    formData.set("organizationId", organizationId)
    formData.set("orgSlug", orgSlug)

    try {
      const res = await updateQueueAction(formData)
      if (res.success) {
        toasts.success("Queue updated successfully!")
        router.refresh()
        onClose()
      } else {
        setError(res.error)
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/40 backdrop-blur-md animate-fade-in">
      <div className="w-full max-w-md bg-card border border-border rounded-2xl shadow-xl overflow-hidden p-6 space-y-4">
        ...
        <div className="flex items-center justify-between border-b border-border pb-3">
          <h2 className="text-lg font-bold text-on-surface">Edit Queue</h2>
          <button
            onClick={onClose}
