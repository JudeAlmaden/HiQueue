...
import { deleteQueueAction } from "@/server/actions/queue.action"
...
export function DeleteQueueDialog({ queueId, queueName, organizationId, orgSlug, onClose, redirectOnSuccess = false }: Props) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      const res = await deleteQueueAction({
        id: queueId,
        organizationId,
        orgSlug,
      })
      ...
      if (res.success) {
        toasts.success("Queue deleted successfully!")
        onClose()
        if (redirectOnSuccess) {
          router.push(`/dashboard/organizations/${orgSlug}/queues`)
        } else {
          router.refresh()
        }
      } else {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/40 backdrop-blur-md animate-fade-in">
      <div className="w-full max-w-sm bg-card border border-border rounded-2xl shadow-xl overflow-hidden p-6 space-y-4">
        ...
        <div className="rounded-xl bg-surface-low p-3.5 border border-border/40 text-[11px] text-on-surface-variant leading-relaxed">
          <span className="font-semibold text-on-surface">Warning:</span> This will permanently remove all associated services, counters, and history. This action cannot be undone, and will fail if there are active tickets in progress.
        </div>
