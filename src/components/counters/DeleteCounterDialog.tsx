...
import { deleteCounterAction } from "@/server/actions/counter.action"
...
export function DeleteCounterDialog({ counterId, counterName, queueId, organizationId, orgSlug, onClose }: Props) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      const res = await deleteCounterAction({
        id: counterId,
        queueId,
        organizationId,
        orgSlug,
      ...
      if (res.success) {
        toasts.success("Counter deleted successfully!")
        router.refresh()
        onClose()
      } else {
        setError(res.error)
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/40 backdrop-blur-md animate-fade-in">
      <div className="w-full max-w-sm bg-card border border-border rounded-2xl shadow-xl overflow-hidden p-6 space-y-4">
        ...
        <div className="rounded-xl bg-surface-low p-3.5 border border-border/40 text-[11px] text-on-surface-variant leading-relaxed">
          <span className="font-semibold text-on-surface">Warning:</span> This will permanently remove the counter, and will fail if there are staff members currently assigned to it.
        </div>
