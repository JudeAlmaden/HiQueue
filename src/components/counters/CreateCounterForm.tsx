...
import { createCounterAction } from "@/server/actions/counter.action"
...
export function CreateCounterForm({ queueId, services, organizationId, orgSlug, onClose }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await createCounterAction(formData)
      if (res.success) {
        toasts.success("Counter added successfully!")
        router.refresh()
        onClose()
      } else {
        setError(res.error)
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/40 backdrop-blur-md animate-fade-in">
      <div className="w-full max-w-md bg-card border border-border rounded-2xl shadow-xl overflow-hidden p-6 space-y-4">
        ...
        <div className="flex items-center justify-between border-b border-border pb-3">
          <h2 className="text-lg font-bold text-on-surface">Add Queue Counter</h2>
          <button
            onClick={onClose}
